C4A-network.RData and C4A-network.xls are the same files but are in different formats, both of which contain Pearson's correlation coefficient (PCC) between C4A and every other feature across different copy numbers in the PsychENCODE dataset.


Raw count and meta- data from the GTEx v7 can be downloaded at the following url:

http://www.gtexportal.org/home/datasets

The data directory should contain the following:

data/:
    GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_reads.gct.gz
    GTEx_Analysis_2016-01-15_v7_RSEMv1.2.22_transcript_expected_count.txt.gz
    phs000424.v7.pht002743.v7.p2.c1.GTEx_Sample_Attributes.GRU.txt
    phs000424.v7.pht002742.v7.p2.c1.GTEx_Subject_Phenotypes.GRU.txt
    GTEx_Analysis_v7_eQTL_expression_matrices/
    GTEx_Analysis_v7_eQTL_covariates